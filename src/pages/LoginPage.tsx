...
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const handleLogin = async () => {
    ...
    setError("");
    try {
      const user = await invoke("sign_in_with_google");
      onLogin(user);
    } catch (e: any) {
      setError(typeof e === "string" ? e : "Login gagal. Coba lagi.");
      setLoading(false);
    }
  };
    ...
    <div className="login-page">
      <div className="login-card fade-in">
        ...
        <button className="btn-google" onClick={handleLogin} disabled={loading}>
          <GoogleIcon />
          {loading ? "Menunggu browser..." : "Sign in with Google"}
        </button>

        <p className="login-note">
          Dengan masuk, kamu menyetujui penggunaan data akun Google<br />
          untuk autentikasi di aplikasi ini.
        </p>
      </div>
    </div>
  );
